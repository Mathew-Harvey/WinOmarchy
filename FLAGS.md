...
## FLAG-64: antivirus flags Winmarchy, and it is not wrong to be suspicious

Context: users report Windows Defender flagging Winmarchy as malicious.

The honest assessment is that the behaviour profile earns it. A system-wide
WH_KEYBOARD_LL hook that swallows and synthesises keystrokes
(chooser/WinKeyGuard.cs), running in a windowless process started from an
HKCU Run key, alongside launches of powershell.exe with -ExecutionPolicy
Bypass and a set of registry personalisation writes, is close to a textbook
description of a keylogger with persistence. A heuristic engine has no way to
know the hook only ever acts on one key.

What was NOT done, and will not be: obfuscation, packing, renaming to look
innocuous, or anything else aimed at making a scanner miss it. That would be
wrong for software people run at login and it stops working at the next
signature update anyway. The remediation here is transparency and identity.

What was done:

1. docs/defender.md documents every triggering behaviour, where it lives,
   why it exists, and what the keyboard hook demonstrably does not do (no
   buffer, no storage, no network code in the process, disarmed outside
   Omarchy mode, dies with the tray). It gives the Microsoft false-positive
   submission route as the fix worth doing, signing as the durable fix, and
   an exclusion only as a stopgap with its cost stated. It also tells a user
   how to check whether the detection might be REAL, which matters more than
   reassuring them.
2. The executable now carries truthful version metadata: product, company,
   description, copyright, version. An anonymous binary with no publisher
   that hooks the keyboard and starts at login has the worst possible
   reputation profile, and saying plainly what it is costs nothing.
3. The README says the same thing briefly and points at the document.

The strongest fact in our favour is structural and verifiable: the
repository ships NO executables. The only committed binary is an icon
generated by tools/make-icon.py. The flagged file is compiled on the user's
own machine by dotnet publish, so a detection can only ever be a judgement
about behaviour, never about a known bad file. Kept that way deliberately.

Outstanding, and it is the maintainer's call because it costs money and needs
a real identity: code signing. It is the only thing that fully fixes the
reputation problem, and signing the PowerShell too would remove the
-ExecutionPolicy Bypass flag from every launch, which is the other large
heuristic trigger.
